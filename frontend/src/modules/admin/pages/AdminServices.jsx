...
const initialServices = [
  { id: "SRV-1", name: "AC Repair & Gas Refill", category: "Home Appliances", providers: 42, isVisible: true, price: 599, time: "90 min" },
  { id: "SRV-2", name: "Men's Haircut + Beard", category: "Salon", providers: 89, isVisible: true, price: 299, time: "45 min" },
  { id: "SRV-3", name: "Tap Fix", category: "Plumbing", providers: 24, isVisible: true, price: 199, time: "30 min" },
  { id: "SRV-4", name: "Full Home Deep Cleaning", category: "Cleaning", providers: 15, isVisible: false, price: 1999, time: "4 hrs" },
  { id: "SRV-5", name: "Switch & Socket Repair", category: "Electrician", providers: 56, isVisible: true, price: 149, time: "20 min" },
];

const initialProducts = [
  { id: "PRD-1", name: "Salon Professional Kit", category: "Salon", price: 999, isVisible: true },
  { id: "PRD-2", name: "AC Filter Cleaner", category: "AC Repair", price: 249, isVisible: true },
];
...
const AdminServices = () => {
  ...
  const handleAddItem = (e) => {
    ...
    setNewItem({ name: "", category: categories[0], price: "", time: "" });
    setEditingItem(null);
    setShowModal(false);
  };
                ...
                {filteredItems.length === 0 ? (
                  ...
                  filteredItems.map(item => (
                    <motion.tr key={item.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`hover:bg-gray-50/50 transition-colors ${!item.isVisible ? 'bg-gray-50/50 grayscale-[50%]' : ''}`}>
                      ...
                      <td className="py-4 px-6">
                        <span className="inline-flex rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-bold text-blue-700 border border-blue-100 uppercase tracking-widest">{item.category}</span>
                      </td>
                      <td className="py-4 px-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      {activeTab === "services" && (
                        <div>
                           ...
                           <input type="text" placeholder="45 min" value={newItem.time} onChange={e => setNewItem({...newItem, time: e.target.value})} className="block w-full rounded-xl border border-gray-200 bg-gray-50 py-1.5 px-4 text-sm font-semibold focus:border-emerald-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
                        </div>
                      )}
                    </div>
